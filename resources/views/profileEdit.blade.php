        ...
        <div class="w-full rounded-lg  md:mt-0 sm:max-w-md ">
            ...
            <div class="space-y-4 md:space-y-6" >
                <div>
                    ...
                    <input required type="text" name="name" id="username" autocomplete="name" value="{{ Auth::user()->name }}" class=" bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " >
                    <input type="hidden" name="id"  value="{{ Auth::user()->id }}" />
                </div>
                <div>
                    <label for="email" class="block mb-2 text-sm font-medium text-gray-900 ">Электронная почта</label>
                    <input required id="email" name="email" type="email" value="{{ Auth::user()->email }}" autocomplete="email" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                </div>
                <div>
                    <label for="phone" class="block mb-2 text-sm font-medium text-gray-900 ">Телефон</label>
                    <input required type="text"  name="phone" id="phone" class="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5" value="{{ Auth::user()->phone}}">
                </div>
